...
export function Palette() {
  return (
    <aside
      data-onboarding-target="palette"
      className="flex h-full w-[244px] shrink-0 flex-col border-r border-line bg-paper"
    >
        ...
        {ORDER.map((kind) => {
          ...
          return (
            <div
              key={kind}
              draggable
              data-onboarding-target={`palette-${kind}`}
              onDragStart={(event) => {
      ...
      <div className="mt-auto border-t border-line px-5 py-3">
        <p className="text-[11px] text-ink-3 leading-relaxed">
          Drag a node onto the canvas to add a step. Every edge you draw is
